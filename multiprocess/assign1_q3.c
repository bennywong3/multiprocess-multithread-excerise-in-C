...
int main(int argc, char* argv[]) 
{
	...
	MY_ARRAY = shmat(shmid, 0, 0);
	for (int i = 0; i < num_integers; i++){//copy contents to MY_ARRAY
		MY_ARRAY[i]=pInputArray[i];
	}



	struct timespec start, end;
	printf("Start timing recursiveMergesort...\n");
	clock_gettime(CLOCK_MONOTONIC_RAW, &start);

	//sleep(1);  // This line is only for test. Remove this line when you implement your solution
	//mergesort4Way4Processes(MyArray,0,num_integers);
	recursiveMergesort(MY_ARRAY,0,num_integers,max_num);
	...
	printf("Start timing bubble_sort...\n");
	clock_gettime(CLOCK_MONOTONIC_RAW, &start);
	...
	//uint64_t delta_ms = (end.tv_sec - start.tv_sec) * 1.0e3 + (end.tv_nsec - start.tv_nsec) * 1.0e-6;
	//printf("The elapsed time (ms) is %lu \n", delta_ms);
	uint64_t delta_us2 = (end.tv_sec - start.tv_sec) * 1.0e6 + (end.tv_nsec - start.tv_nsec) * 1.0e-3;
	printf("The elapsed time (us) is %lu \n", delta_us2);
	// uint64_t delta_s = (end.tv_sec - start.tv_sec);
	// printf("The elapsed time (s) is %lu \n", delta_s);

	verifySortResults(pInputArray, MY_ARRAY, num_integers); // Replace YOUR_ARRAY by your array name
